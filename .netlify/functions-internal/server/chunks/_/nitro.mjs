...
function getQuery$3(input) {
  return parseQuery$3(parseURL$3(input).search);
}
...
function parseURL$3(input = "", defaultProto) {
  ...
  if (!hasProtocol$3(input, { acceptRelative: true })) {
    return defaultProto ? parseURL$3(defaultProto + input) : parsePath$3(input);
  }
  ...
  const { pathname, search, hash } = parsePath$3(path);
  return {
    ...
    for (const layer of stack) {
      ...
      if (res.handler.__resolve__) {
        ...
        res = {
          ...res,
          ..._res,
          route: joinURL$3(res.route || "/", _res.route || "/")
        };
      }
      return res;
    }
...
function websocketOptions(evResolver, appOptions) {
  return {
    ...
    async resolve(info) {
      ...
      const { pathname } = typeof url === "string" ? parseURL$3(url) : url;
...
function withBase(input, base) {
  if (isEmptyURL$2(base) || hasProtocol$2(input)) {
    return input;
  }
...
function withoutBase$2(input, base) {
  if (isEmptyURL$2(base)) {
    return input;
  }
  const _base = withoutTrailingSlash$2(base);
  if (!input.startsWith(_base)) {
...
function withProtocol(input, protocol) {
  let match = input.match(PROTOCOL_REGEX$2);
  if (!match) {
    match = input.match(/^\/{2,}/);
  }
  if (!match) {
...
function parseURL$2(input = "", defaultProto) {
  ...
  const { pathname, search, hash } = parsePath$2(path);
  return {
    protocol: protocol.toLowerCase(),
    auth: auth ? auth.slice(0, Math.max(0, auth.length - 1)) : "",
    host,
  ...
  const $fetchRaw = async function $fetchRaw2(_request, _options = {}) {
    ...
    if (typeof context.request === "string") {
      ...
      if (context.options.query) {
        context.request = withQuery$2(context.request, context.options.query);
...
const _inlineRuntimeConfig = {
  "app": {
    "baseURL": "/",
    "buildId": "02b79df1-5214-45c4-a278-810bec7b022b",
  ...
  "public": {
    "STORYBLOK_KEY": "HrmL9nir5XWr8PtWRrj3mQtt",
    "storyblok": {
      "accessToken": "HrmL9nir5XWr8PtWRrj3mQtt",
...
function normalizeSiteConfig(config) {
  ...
  if (config.url && !hasProtocol$2(config.url, { acceptRelative: true, strict: false }))
...
function resolveSitePath(pathOrUrl, options) {
  let path = pathOrUrl;
  if (hasProtocol$2(pathOrUrl, { strict: false, acceptRelative: true })) {
    const parsed = parseURL$2(pathOrUrl);
  ...
  let origin = withoutTrailingSlash$2(options.absolute ? options.siteUrl : "");
  if (base !== "/" && origin.endsWith(base)) {
    origin = origin.slice(0, origin.indexOf(base));
  }
  const baseWithOrigin = options.withBase ? withBase(base, origin || "/") : origin;
  const resolvedUrl = withBase(path, baseWithOrigin);
  return path === "/" && !options.withBase ? withTrailingSlash$2(resolvedUrl) : fixSlashes(options.trailingSlash, resolvedUrl);
}
...
function fixSlashes(trailingSlash, pathOrUrl) {
  const $url = parseURL$2(pathOrUrl);
  if (isPathFile($url.pathname))
    return pathOrUrl;
  const fixedPath = trailingSlash ? withTrailingSlash$2($url.pathname) : withoutTrailingSlash$2($url.pathname);
...
const _rmSz4KnaKJkj52qaBWD3kAcH2g_v1x1S9vSubVH_9Q = defineNitroPlugin(async (nitroApp) => {
...
const plugins = [
  _rmSz4KnaKJkj52qaBWD3kAcH2g_v1x1S9vSubVH_9Q
];

const _APBEDA = defineEventHandler(async (e) => {
...
const __yN6n5 = defineEventHandler(async (e) => {
  ...
  const isNotIndexButHasIndex = referrer !== fixPath("/sitemap.xml") && parseURL$2(referrer).pathname.endsWith("-sitemap.xml");
  const sitemapName = parseURL$2(referrer).pathname.split("/").pop()?.split("-sitemap")[0] || fallbackSitemapName;
  ...
  const conditionalTips = [
    ...
    `URLs missing? Check Nuxt Devtools Sitemap tab (or the <a href="${withQuery$2("/__sitemap__/debug.json", { sitemap: sitemapName })}" style="color: #398465" target="_blank">debug endpoint</a>).`
  ];
  if (!isShowingCanonical) {
    const canonicalPreviewUrl = withQuery$2(referrer, { canonical: "" });
...
function resolve(s, resolvers) {
  ...
  if (hasProtocol$2(s, { acceptRelative: true, strict: false }))
...
function filterSitemapUrls(_urls, options) {
  ...
  return _urls.filter((e) => {
    let path = e.loc;
    try {
      path = parseURL$2(e.loc).pathname;
    } catch {
      return false;
    }
    if (!urlFilter(path))
  ...
  if (autoI18n && isI18nMapped) {
    return sources.map((s) => {
      const urls = (s.urls || []).map((_url) => {
        ...
        if (!hasProtocol$2(url.loc, { acceptRelative: true }))
      ...
      s.urls = urls.map((url) => {
        if (url._sitemap || url._i18nTransform)
          return url;
        if (url.loc && !hasProtocol$2(url.loc, { acceptRelative: true })) {
...
function applyI18nEnhancements(_urls, options) {
  ...
  return _urls.map((e) => {
    ...
    const parsedURL = parseURL$2(e.loc);
      ...
      return {
        ...
        alternatives: [
          ...
          return {
            ...
            href: joinURL$2(withHttps(locale2.domain), pathWithoutLocale)
          };
        })
      };
    }
    return autoI18n.locales.map((l) => {
      let loc = joinURL$2(`/${l.code}`, pathWithoutLocale);
      ...
      return {
        ...
        alternatives: [{ code: "x-default" }, ...autoI18n.locales].map((locale2) => {
          ...
          if (autoI18n.strategy === "prefix") {
            href = joinURL$2("/", code, pathWithoutLocale);
...
function createNitroRouteRuleMatcher() {
  ...
  return (path) => {
    return defu({}, ..._routeRulesMatcher.matchAll(
      ...
      withoutBase$2(withoutTrailingSlash$2(withoutQuery(path)), app.baseURL)
    ).reverse());
  };
}
...
async function buildSitemap(sitemap, resolvers, runtimeConfig) {
  ...
  let enhancedUrls = normalisedUrls.map((e) => defu(e, sitemap.defaults)).map((e) => {
    const path = parseURL$2(e.loc).pathname;
...
const _HZnI_I = defineEventHandler(async (e) => {
...
const _lazy_uOzIQL = () => import('../routes/renderer.mjs').then(function (n) { return n.r; });

const handlers = [
  { route: '/__nuxt_error', handler: _lazy_uOzIQL, lazy: true, middleware: false, method: undefined },
  { route: '', handler: _APBEDA, lazy: false, middleware: true, method: undefined },
  { route: '/__sitemap__/style.xsl', handler: __yN6n5, lazy: false, middleware: false, method: undefined },
  { route: '/sitemap.xml', handler: _HZnI_I, lazy: false, middleware: false, method: undefined },
  { route: '/__nuxt_island/**', handler: _SxA8c9, lazy: false, middleware: false, method: undefined },
  { route: '/**', handler: _lazy_uOzIQL, lazy: true, middleware: false, method: undefined }
];
...
export { $fetch as $, createRouter$1 as A, defu as B, useRuntimeConfig as a, buildAssetsURL as b, getResponseStatusText as c, getResponseStatus as d, defineRenderHandler as e, getQuery$2 as f, getRouteRulesForPath as g, createError$1 as h, getRouteRules as i, joinHeaders as j, parseQuery as k, hasProtocol as l, joinURL as m, normalizeCookieHeader as n, getContext as o, publicAssetsURL as p, withTrailingSlash as q, withoutTrailingSlash as r, isScriptProtocol as s, sanitizeStatusCode as t, useNitroApp as u, baseURL as v, withQuery as w, createHooks as x, executeAsync as y, toRouteMatcher as z };
